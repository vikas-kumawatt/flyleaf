...
- **Gestures never worked on a device until 26 Sep** (found at the 07b rebuild). The app had no `GestureHandlerRootView` at its root, so `react-native-gesture-handler` ignored every gesture: the progress slider (`ProgressSlider`, SL-52), the gesture in `components.tsx`, and the feed-card swipes. It's fixed in `app/_layout.tsx`, pinned by `root-layout.test.ts`, and needed no rebuild. On the phone, confirm the slider drags and saves once per drag, and record that nothing tested this before.
