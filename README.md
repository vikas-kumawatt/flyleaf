...
**Social Graph, Follows, Blocking & Muting (SO-01, SO-02, SO-03, SO-04):**
...
- Follow, pending request, blocking, and muting API routes (`POST /v1/users/:id/follow`, `DELETE /v1/users/:id/follow`, `GET /v1/me/follow-requests`, `POST /v1/users/:id/block`, `DELETE /v1/users/:id/block`, `GET /v1/me/blocks`, `POST /v1/users/:id/mute`, `DELETE /v1/users/:id/mute`, `POST /v1/works/:id/mute`, `DELETE /v1/works/:id/mute`, `GET /v1/me/mutes`) synchronized with 0 OpenAPI contract drift.
...
| `vitest run` (API) | **572 tests** across **27 test suites** — identity (47), schema (29), search (33), ingest (65), outbound (27), jobs (10), relevance (10), dedupe (38), ISBN (18), authorization (43), contract (8), hooks (17), admin (22), admin-catalog (7), reading (10), reviews (12), profile-stats (8), telemetry (7), shelves-migration (11), shelves (36), shelves-privacy (20), imports-migration (12), imports (16), import-mapping (37), import-matcher (12), import-committer (14), import-processor (4). Every suite that touches SQL runs against **PGlite** (Postgres compiled to WASM) |
