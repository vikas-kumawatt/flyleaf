...
**Social Graph, Follows & Blocking (SO-01, SO-02, SO-03):**
...
- Follow, pending request, and blocking API routes (`POST /v1/users/:id/follow`, `DELETE /v1/users/:id/follow`, `GET /v1/me/follow-requests`, `POST /v1/users/:id/block`, `DELETE /v1/users/:id/block`, `GET /v1/me/blocks`) synchronized with 0 OpenAPI contract drift.
...
- Mobile screens: `UserProfileScreen` (`apps/mobile/app/user/[id].tsx`) with haptic follow toggle, mutual follow indicator ("Follows you"), Block action, and restricted private view; `FollowRequestsScreen` (`apps/mobile/app/profile/requests.tsx`) for managing incoming follow requests; `BlockedUsersScreen` (`apps/mobile/app/profile/blocked.tsx`) for viewing and unblocking accounts.
